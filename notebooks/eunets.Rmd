...
number_participants <- count(collaborations, id) # allowing same country repeats!
country_selection <- c(eu15, eu13, other) # in `R/global.R`

collaborations <- collaborations %>%
  # remove single-participant projects
  semi_join(filter(number_participants, n > 1), by = "id") %>%
  # restrict to country selection
...
point_coordinates <-
  igraph::layout_with_fr(g, weights = edge_weight ^ (1 / 5))
...
ggplot() +
  ...
  scale_color_gradient(low = rgb(.9, .9, .9, 1), high = rgb(.5, .5, .5, 1)) +
  geom_point(
    data = nodes,
