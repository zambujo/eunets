...
df %>% 
  count(funding_scheme, call, programme, sort = TRUE) %>%
  filter(str_detect(call, regex("euratom", ignore_case = TRUE))) %>%
  ## filter(str_detect(programme, "H2020-EU\\.2\\.1\\.[;]|H2020-EU\\.2\\.1\\.$")) %>%
  print(n = 60)
