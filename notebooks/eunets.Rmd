...
```{r df-to-graph, message=FALSE, warning=FALSE}
h2020 <-
  here("data", "h2020.csv") %>%
  read_csv()
  # mutate(group = fct_relevel(group, "eu15", "eu13", "ac", "tc"))

h2020 <- h2020 %>%
  # remove single-country projects and other countries
  filter(group %in% c("eu15", "eu13", "ac"), nunique > 1)

info_countries <- h2020 %>%
...
```{r plot-raw-graph, fig.width=9, fig.height=9, message=FALSE, warning=FALSE}
ggplot() +
  geom_segment(
    data = edges,
    aes(
...
```{r raw-log-scale, fig.width=9, fig.height=9, message=FALSE, warning=FALSE}
...
```{r contrib-graph, message=FALSE, warning=FALSE}
pivot <- h2020 %>%
  group_by(rcn, country) %>%
...
```{r plot-contrib-graph, fig.width=9, fig.height=9, message=FALSE, warning=FALSE}
ggplot() +
  geom_segment(
    data = edges,
    aes(
