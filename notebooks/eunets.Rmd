...
if (update_data) {
  # 67.4 MB CSV file (last updated on 2020-12-14)
  download.file(
    "https://cordis.europa.eu/data/cordis-h2020projects.csv",
    here("data-raw", "cordis-h2020projects.csv"))
  download.file(
    "http://cordis.europa.eu/data/reference/cordisref-countries.csv",
    here("data-raw", "cordisref-countries.csv"))
  download.file(
    "http://cordis.europa.eu/data/reference/cordisref-sicCode.csv",
    here("data-raw", "cordisref-sicCode.csv"))
  download.file(
    "http://cordis.europa.eu/data/reference/cordisref-projectFundingSchemeCategory.csv",
    here("data-raw", "cordisref-projectFundingSchemeCategory.csv"))
}
df <-
  here("data-raw", "cordis-h2020projects.csv") %>%
  read_csv2() %>%
...
population_table <- wbstats::wb_data(
  ...
  country = countrycode(country_selection, "eurostat", "iso2c"),
  start_date = 2017,
  end_date = 2017) %>%
  clean_names() %>%
